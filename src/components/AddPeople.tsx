...
export const AddPeople = ({
  ...
  return (
    <div className="flex flex-row justify-center gap-3 sm:gap-5">
      <input
        className=" outline-none border-2 rounded-sm border-cyan-900 sm:px-1 "
        placeholder="type name"
        value={personName}
        onChange={(e) => setPersonName(e.target.value)}
      />
      <button
        className="sm:px-5 p-1 rounded-sm bg-cyan-100 hover:bg-cyan-50 border hover:bg-gradient-to-r from-cyan-100 to-green-50"
        onClick={() => {
